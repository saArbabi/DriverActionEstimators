...
class DataGeneratorLaneKeep:
    ...
    def initiate(self):
        self.env.usage = 'data generation'
        self.env.recordings = {}
        self.env.veh_log = ['glob_x','speed', 'act_long']
        self.indxs = {}
    ...
    def get_step_feature(self, e_veh, f_veh):
        if not f_veh:
            f_veh = {key: np.nan for key in self.env.veh_log}
            f_veh_exists = 0
        else:
        ...
        e_veh_aggressiveness = e_veh['aggressiveness']
        step_feature = [e_veh_aggressiveness, f_veh_exists,
                        e_veh['glob_x'], f_veh['glob_x'],
                        e_veh['speed'], f_veh['speed'],
                        e_veh['act_long'], f_veh['act_long']]
    ...
    def extract_features(self, raw_recordings):
        def add_info(f_veh_id, time_step):
            """Useful for debugging
            """
            f_veh_id = f_veh_id if f_veh_id else -1
            return [episode_id, time_step, e_veh_id, f_veh_id]
        ...
        for e_veh_id in vehicle_ids:
            ...
            for time_step, e_veh in e_veh_ts.items():
                ...
                step_feature = self.get_step_feature(e_veh, f_veh)
                step_feature[0:0] = add_info(f_veh_id, time_step)
                epis_features.append(step_feature)
        return np.array(features)
    ...
    def fill_missing_values(self, features):
        ...
        cols_with_nans = ['f_veh_speed', 'f_veh_action',
                          'el_delta_v', 'el_delta_x', 'f_veh_glob_x']
        for col in cols_with_nans:
            fill_with_dummy(self.indxs[col])
    ...
    def split_data(self, history_future_seqs, history_future_seqs_scaled):
        ...
        # future action of m_veh - fed to LSTMs
        col_names = ['episode_id', 'time_step', 'f_veh_action']
        future_m_veh_a = future_seqs_scaled[:, :, self.names_to_index(col_names)]
    ...
    def scale_data(self, features):
        col_names = ['f_veh_speed', 'e_veh_speed',
                        'el_delta_v', 'el_delta_x']
